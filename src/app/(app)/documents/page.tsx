...
const DocumentTable = ({ docs, uploadedData, title, description, icon: Icon }: any) => (
                        ...
                        {docs.map((doc: any) => {
                            ...
                            return (
                                <TableRow key={doc.id} className="border-white/10">
                                    ...
                                    <TableCell className="text-xs text-slateMuted">
                                        {docData?.files?.length > 0 ? (
                                            <ul className="space-y-1.5">
                                                {docData.files.map((file: any) => {
                                                    ...
                                                    return (
                                                        <li key={file.date} className="font-mono flex items-center gap-2">
                                                            ...
                                                            <span className="text-slateMuted">{format(new Date(file.date), 'MMM d, yyyy')}</span>
...
function DocumentsPageContent() {
    ...
    return (
        <main className="flex-1 space-y-8 p-4 md:p-8">
            <div>
                <h1 className="text-3xl font-display tracking-tight">Document Locker</h1>
                <p className="text-slateMuted mt-1">Your central hub for all application-related documents.</p>
            </div>
            ...
            <Card className="bg-gradient-to-r from-blue/10 to-transparent border-blue/20">
                <CardHeader className="flex-row items-center gap-6">
                    ...
                    <div>
                        <CardTitle className="font-display">Manage Your Documents</CardTitle>
                        <CardDescription className="mt-1 text-slateMuted">
