...
const CollegeResults = ({ title, colleges, studentProfile, filteringLogic, searchTerm, icon: Icon, isMatch = true }: any) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <h2 className="flex items-center gap-3 font-display text-3xl mb-6 tracking-tight"><Icon className="h-8 w-8 text-blue" />{title} <span className="text-slateMuted font-body font-normal text-2xl">({filteredColleges.length})</span></h2>
      ...
      <div className="flex-1 space-y-8 p-4 md:p-8">
        <div className="text-center">
            <div className="inline-block p-4 bg-blue/10 rounded-full mb-4">
                <WandSparkles className="h-16 w-16 text-blue" />
            </div>
            <h1 className="text-4xl font-display tracking-tight">AI College Finder</h1>
            <p className="text-slateMuted mt-2 max-w-2xl mx-auto text-lg">Tell us your preferences, and our AI will find the best-fit institutions for you from our curated list of top Canadian DLIs.</p>
        </div>

        <Card className="bg-surface1 border-white/10 shadow-xl">
            <CardHeader>
                <CardTitle className="text-2xl font-display">Your Search Criteria</CardTitle>
                <CardDescription className="text-slateMuted">Adjust the filters below to refine your search for the perfect school.</CardDescription>
            </CardHeader>
            <CardContent>
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
                    <div className="space-y-2 lg:col-span-2">
                        ...
                        <Input id="fieldOfInterest" value={fieldOfInterest} onChange={e => setFieldOfInterest(e.target.value)} placeholder="e.g., Computer Science, Healthcare" required className="py-6 text-base"/>
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="province">Province</Label>
                        <Select value={province} onValueChange={setProvince}>
                            <SelectTrigger id="province" className="py-6 text-base"><SelectValue placeholder="All Provinces" /></SelectTrigger>
                            <SelectContent>
                    ...
                    <div className="space-y-2">
                        <Label>Max Annual Tuition: <span className="font-semibold text-blue">{formatCurrency(maxTuition)}</span></Label>
                        <Slider defaultValue={[maxTuition]} max={100000} step={1000} onValueChange={(v) => setMaxTuition(v[0])} />
                    </div>
                    <Button type="submit" disabled={loading || !fieldOfInterest} className="w-full md:col-span-2 lg:col-span-4 text-lg py-7 bg-red hover:bg-red/90 font-bold" size="lg">
        ...
        {error && <Card className="bg-surface1"><CardContent className="p-8 text-center text-red">{error}</CardContent></Card>}
        
        <AnimatePresence>
            {results && (results.universities.length > 0 || results.colleges.length > 0) && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
                    <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slateMuted" /><Input placeholder="Filter recommended schools by name..." className="pl-10 text-base py-6 bg-surface1" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
                    ...
                    {suggestions && (
                        <div className="bg-surface2 p-6 rounded-lg space-y-4">
                            <h3 className="font-bold text-lg flex items-center gap-2"><Lightbulb className="h-5 w-5 text-blue" />Additional Recommendations</h3>
                            {suggestions.bridgingCourses.length > 0 && (
                                <div>
                                    <p className="font-semibold">Bridging Courses</p>
                                    <ul className="list-disc list-inside text-sm text-slateMuted">
            ...
            {results && results.universities.length === 0 && results.colleges.length === 0 && (
                 <Card className="text-center py-16 bg-surface1">
                     <CardContent className="flex flex-col items-center gap-4">
                         <Frown className="h-16 w-16 text-slateMuted/50" />
                         <h3 className="text-2xl font-display">No Matches Found</h3>
                         <p className="text-slateMuted max-w-md text-lg">We couldn't find any schools matching your criteria. Try adjusting your tuition budget or broadening your field of interest.</p>
                     </CardContent>
                 </Card>
            )}
        </AnimatePresence>
...
export default function CollegeMatchPage() {
  return <CollegeMatchPageContent />;
}
