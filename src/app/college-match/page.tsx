...
const CollegeResults = ({ title, colleges, studentProfile, filteringLogic, searchTerm, icon: Icon, isMatch = true }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <h2 className="flex items-center gap-3 font-black text-3xl mb-6 tracking-tight"><Icon className="h-8 w-8 text-primary" />{title} <span className="text-muted-foreground font-normal text-2xl">({filteredColleges.length})</span></h2>
      ...
      <div className="flex-1 space-y-8 p-4 md:p-8">
        <div className="text-center">
            <div className="inline-block p-4 bg-primary/10 rounded-full mb-4">
                <WandSparkles className="h-16 w-16 text-primary" />
            </div>
            <h1 className="text-4xl font-black tracking-tighter">AI College Finder</h1>
            <p className="text-muted-foreground mt-2 max-w-2xl mx-auto text-lg">Tell us your preferences, and our AI will find the best-fit institutions for you from our curated list of top Canadian DLIs.</p>
        </div>

        <Card className="border-border/50 shadow-xl">
            <CardHeader>
                <CardTitle className="text-2xl font-bold">Your Search Criteria</CardTitle>
        ...
        <AnimatePresence>
            {results && (results.universities.length > 0 || results.colleges.length > 0) && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
                    <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /><Input placeholder="Filter recommended schools by name..." className="pl-10 text-base py-6" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
            ...
            {results && results.universities.length === 0 && results.colleges.length === 0 && (
                 <Card className="text-center py-16">
                     <CardContent className="flex flex-col items-center gap-4">
                         ...
                         <p className="text-muted-foreground max-w-md text-lg">We couldn't find any schools matching your criteria. Try adjusting your tuition budget or broadening your field of interest.</p>
                     </CardContent>
                 </Card>
            )}
        </AnimatePresence>
